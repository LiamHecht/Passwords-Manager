...
cnt.execute('''CREATE TABLE IF NOT EXISTS configuration ( website TEXT, email TEXT, password TEXT)''')
cnt.cursor()
...
def insert_details(website,email,password):
	cnt.execute("INSERT INTO configuration VALUES (:website, :email, :password)", {
		'website': website,
		'email': email,
		'password': password
	})
